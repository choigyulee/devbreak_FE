...
import getBlogBlogId from "../../APIs/get/getBlogBlogId"; // 블로그 데이터 API
import getIssuesAndCommits from "../../APIs/get/getIssuesAndCommits"; // 이슈 및 커밋 데이터 API

function BlogPage() {
  const { blogId } = useParams(); // URL에서 blogId를 받아옴
  const navigate = useNavigate();
  ...
  const fetchIssuesAndCommits = async () => {
    try {
      const data = await getIssuesAndCommits(); // 이슈 및 커밋 데이터 API 호출
  ...
  useEffect(() => {
    if (blogId) {
      fetchBlogData(blogId); // 블로그 데이터를 가져옴
    }
    fetchIssuesAndCommits(); // 이슈 및 커밋 데이터를 가져옴
  }, [blogId]); // blogId가 바뀔 때마다 다시 실행
      ...
      <Container>
        <BlogInfo>
          ...
          <DescriptionContainer>
            <BlogDescription>
              {blogData.description}
              <IconContainer onClick={() => window.open(blogData.git_repo_url, "_blank")}>
                <BsGithub size={24} />
              </IconContainer>
            </BlogDescription>
          </DescriptionContainer>
        ...
        {/* 이슈 및 커밋 목록 */}
        <Section>
          <SectionTitle>Issues & Commits</SectionTitle>
          <ActivityContainer>
            {issuesAndCommits.length === 0 ? (
              <p>No issues or commits found.</p>
            ) : (
              issuesAndCommits.map((item, index) => (
