...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response && error.response.status === 401 && !originalRequest._retry) {
      ...
      try {
        console.log('액세스 토큰 갱신 시도 중...');
        await postAuthRefresh(); // 액세스 토큰 갱신
        processQueue(null); // 대기 중인 요청 처리
        return axiosInstance(originalRequest); // 원래 요청 재시도
      } catch (refreshError) {
        console.error('리프레시 토큰 갱신 오류:', refreshError.response?.data || refreshError.message);
        ...
        // 리프레시 토큰 갱신 실패 시 강제로 로그아웃 처리
        await postAuthLogout(); // 세션 만료 처리
        alert('세션이 만료되었습니다. 다시 로그인하세요.'); // 세션 만료 메시지

        // 리로드하여 로그아웃 상태로 페이지 새로고침
        window.location.href = '/login'; // 로그인 페이지로 리다이렉트
        return Promise.reject(refreshError); // 리프레시 실패 처리
      } finally {
        isRefreshing = false; // 리프레시 상태 해제
      }
    }
